...
export class PostgresBattleDatasource implements BattleDatasource{
    ...
    async recordTurn(battleId: number, attackerId: number, moveId: number): Promise<Battle> {
        ...
        return await prisma.$transaction(async (tx: Omit<PrismaClient, "$connect" | "$disconnect" | "$on" | "$transaction" | "$use" | "$extends">) => {
            ...
            const attacker = await tx.pokemon.findUnique({ where: { id: attackerId }});
            const defender = await tx.pokemon.findUnique({ where: { id: defenderId }});
            const move = await tx.move.findUnique({ where: { id: moveId }});

            if (!attacker || !defender || !move)
                throw new Error("Invalid turn data");

            const pokemonMove = await tx.pokemonMove.findFirst({
